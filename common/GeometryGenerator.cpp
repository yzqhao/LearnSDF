...
{
	Math::Vec3 p0 = v0.Position;
	Math::Vec3 p1 = v1.Position;

	Math::Vec3 n0 = v0.Normal;
	Math::Vec3 n1 = v1.Normal;

	Math::Vec3 tan0 = v0.TangentU;
	Math::Vec3 tan1 = v1.TangentU;

	Math::Vec2 tex0 = v0.TexC;
	Math::Vec2 tex1 = v1.TexC;

    // Compute the midpoints of all the attributes.  Vectors need to be normalized
    // since linear interpolating can make them not unit length.  
    Math::Vec3 pos = (p0 + p1) * 0.5;
    Math::Vec3 normal = ((n0 + n1) * 0.5f);
	normal.Normalize();
    Math::Vec3 tangent = ((tan0+tan1) * 0.5f);
	tangent.Normalize();
    Math::Vec2 tex = (tex0 + tex1) * 0.5f;
...
GeometryGenerator::MeshData GeometryGenerator::CreateGeosphere(float radius, uint32 numSubdivisions)
{
	...
	for(uint32 i = 0; i < meshData.Vertices.size(); ++i)
	{
		...
		Math::Vec3 n = meshData.Vertices[i].Position.GetNormalized();

		// Project onto sphere.
		Math::Vec3 p = n * radius;
		...
		meshData.Vertices[i].TexC.x = theta/(Math::PI * 2);
...
GeometryGenerator::MeshData GeometryGenerator::CreateCylinder(float bottomRadius, float topRadius, float height, uint32 sliceCount, uint32 stackCount)
{
	...
	for(uint32 i = 0; i < ringCount; ++i)
	{
		...
		for(uint32 j = 0; j <= sliceCount; ++j)
		{
			...
			// This is unit length.
			vertex.TangentU = Math::Vec3(-s, 0.0f, c);

			float dr = bottomRadius-topRadius;
			Math::Vec3 bitangent(dr*c, -height, dr*s);

			Math::Vec3 T = vertex.TangentU;
			Math::Vec3 B = bitangent;
			vertex.Normal = CrossProduct(T, B).GetNormalized();
...
GeometryGenerator::MeshData GeometryGenerator::CreateGrid(float width, float depth, uint32 m, uint32 n)
{
	...
	for(uint32 i = 0; i < m; ++i)
	{
		...
		for(uint32 j = 0; j < n; ++j)
		{
			...
			meshData.Vertices[i * n + j].TangentU = Math::Vec3(1.0f, 0.0f, 0.0f);
