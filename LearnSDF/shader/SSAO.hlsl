...
Texture2D NormalGbuffer : register(t0);
Texture2D DepthGbuffer : register(t1);
...
{	
	...
	// Get viewspace P
    float Pz = DepthGbuffer.SampleLevel(gsamLinearWrap, pin.TexC, 0.0f).r;
	...
	for(int i = 0; i < gSampleCount; ++i)
	{
        ...
        float Rz = DepthGbuffer.SampleLevel(gsamLinearWrap, UV, 0.0f).r;
