...
Texture2D gTextureMaps[200] : register(t0);
...
PixelOutput PS(VertexOut pin) //: SV_Target
{
    ...
    Texture2D BaseColorTexture = gTextureMaps[gMaterialIndex * 4];
    Texture2D NormalTexture = gTextureMaps[gMaterialIndex * 4 + 1];
    Texture2D MetallicTexture = gTextureMaps[gMaterialIndex * 4 + 2];
    Texture2D RoughnessTexture = gTextureMaps[gMaterialIndex * 4 + 3];
