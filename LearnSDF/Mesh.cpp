...
Vertex Mesh::MidPoint(const Vertex& v0, const Vertex& v1)
{
    ...
    Math::Vec3 pos = 0.5f * (v0.Position + v1.Position);
    Math::Vec3 normal = 0.5f * (v0.Normal + v1.Normal);
    normal.Normalize();
    Math::Vec3 tangent = 0.5f * (v0.TangentU + v1.TangentU);
    tangent.Normalize();
    Math::Vec2 tex = 0.5f * (v0.TexC + v1.TexC);
...
void Mesh::CreateGrid(float width, float depth, uint32 m, uint32 n)
{
    ...
    for (uint32 i = 0; i < m; ++i)
    {
        ...
        for (uint32 j = 0; j < n; ++j)
        {
            ...
            Vertices[i * n + j].Position = Math::Vec3(x, 0.0f, z);
            Vertices[i * n + j].Normal = Math::Vec3(0.0f, 1.0f, 0.0f);
            Vertices[i * n + j].TangentU = Math::Vec3(1.0f, 0.0f, 0.0f);
